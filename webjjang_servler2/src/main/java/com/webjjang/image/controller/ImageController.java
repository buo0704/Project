	...
	public String execute(HttpServletRequest request) {
		...
		try { // 정상 처리
			...
			case "/image/list.do":
				...
				// 이미지 게시판의 한페이지 이미지의 개수의 기본값을  6으로 처리하자
				// 중복처리를 하는 것이다. - 앞에 데이터에 덮어쓰기가 된다
			...
			case "/board/view.do":
				...
				// 1. 조회수 1증가(글보기), 2. 이미지 글보기 데이터 가져오기 : 글보기, 글수정
				...
				//댓글 페이지 객체 
				request.setAttribute("replyPageObject", replyPageObject); //0628	
				jsp = "board/view";
				break;
			
			
			case "/image/writeForm.do":
				System.out.println("3.이미지 등록 폼");
				jsp="image/writeForm";
				break;
		
			
			case "/image/write.do":
				System.out.println("3-1.이미지 등록 처리");
				
				//이미지 업로드 처리
				//new MultipartRequest(request, 실저장위치 , 사이즈제한
				// endcoding, 중복처리객체 , 파이름이름뒤 cnt븉임
				...
				session.setAttribute("msg", "글등록 성공");
				break;
			...
			case "/board/update.do":
				System.out.println("4-2.이미지 수정 처리");
				...
				session.setAttribute("msg", "게시판이 수정 되었습니다" );
				break;
			
			
			case "/board/delete.do":
				System.out.println("5.이미지 삭제");
				...
				session.setAttribute("msg", "게시판이 삭제 되었습니다" );
