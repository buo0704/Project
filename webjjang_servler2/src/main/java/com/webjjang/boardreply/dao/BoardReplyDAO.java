...
public class BoardReplyDAO extends DAO {
	...
	public List<BoardReplyVO> list(ReplyPageObject pageObject) throws Exception {
		...
		try {
			...
			// 3. sql - 아래 LIST - 오류가 발생하면 콘솔 확인하고 여기에 쿼리에 해당되는 list 출력해 본다.
			// 4. 실행 객체 & 데이터 세팅
			pstmt = con.prepareStatement(LIST);	
			// 검색 대한 데이터 세팅 - list() 만 사용
			pstmt.setLong(1, pageObject.getNo()); // 기본값 1
	...
	// 3. 글등록 처리
	// BoardReplyController - (Execute) - BoardReplyWriteService - [BoardReplyDAO.increase()]
	public int write(BoardReplyVO vo) throws Exception {
		...
		try {
			...
			// 6. 표시 또는 담기
			System.out.println();
			System.out.println("*** 댓글 완료 되었습니다.");
		} catch (Exception e) {
			e.printStackTrace();
